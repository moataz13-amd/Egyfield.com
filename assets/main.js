  ...
  const L = {
      ar: {
          ...
          filter_fresh: "طازج",
          filter_frozen: "مجمد",
          filter_pickled: "مخللات",
          filter_grains: "حبوب",
          contact_h2: "طلب عرض سعر",
          contact_p: "املأ البيانات وسنتواصل خلال 24 ساعة.",
          ...
          about_h2: "عن إيجي فيلد",
          about_p: "تأسست إيجي فيلد في عام 2015 وتوسعت عالميًا في 2018. ننمو عبر الجودة والثقة والقيمة، ونبني شراكات قوية بينما نقدم منتجات وخدمات مميزة.",
      ...
      en: {
          ...
          filter_pickled: "Pickled",
          filter_grains: "Grains",
          contact_h2: "Request a Quote",
          contact_p: "Fill the form and we'll reply within 24h.",
          form_name: "Company Name",
          form_email: "Email",
          form_country: "Country",
          form_product: "Product",
          form_qty: "Quantity (MT)",
          form_msg: "Message",
          form_submit: "Send Request",
          footer: "© EgyField. All rights reserved.",
          about_h2: "About EgyField",
          about_p: "Founded in 2015, expanded globally in 2018. We grow through quality, trust, and value—building strong partnerships and delivering unique products and services.",
  ...
  function detailsButtonsInit() {
      ...
      cards.forEach(card => {
          ...
          if (!actions) { actions = document.createElement('div');
              actions.className = 'actions-row';
  ...
  function initLang() { const saved = localStorage.getItem('lang'); if (saved) lang = saved;
      setLang(lang); }
  ...
  function ensureMobileMenuScaffold() {
      ...
      // Ensure a close button exists inside the navbar (mobile drawer)
      let closeBtn = navbar.querySelector('.nav-close');
      if (!closeBtn) {
      ...
      // Ensure accessibility attributes on toggle
      if (!toggleBtn.id) toggleBtn.id = 'menu-toggle';
      toggleBtn.setAttribute('aria-controls', 'navbar');
      toggleBtn.setAttribute('aria-expanded', 'false');
      toggleBtn.setAttribute('aria-label', 'Open menu');

      // Ensure global nav overlay exists
  ...
  function closeNavbar() { try { toggleMenu(false); } catch (_) {} }
  ...
  function toggleMenu(open) {
      ...
      const isOpen = (open !== undefined) ? !!open : !nav.classList.contains('show');
      nav.classList.toggle('show', isOpen);
      if (overlay) overlay.classList.toggle('show', isOpen);
      try { document.body.setAttribute('data-nav-open', isOpen ? 'true' : 'false'); } catch (_) {}
      if (toggleBtn) {
  ...
  function rfqInit() {
      const f = document.getElementById('rfq');
      if (!f) return;
      const urlParams = new URLSearchParams(location.search);
      const qp = urlParams.get('product');
      if (qp) { const sel = f.querySelector('select[name=product]'); if (sel) sel.value = qp; }
